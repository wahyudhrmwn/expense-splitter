...
import { Users, Plus, Trash2, Edit, DollarSign, ArrowRight } from 'lucide-react';
...
export default function ExpenseSplitterPage() {
  ...
  }, [expensesCount, peopleCount, groupsLength, selectedGroup, calculateBalances]);
  ...
  const handleAddExpense = (e: React.FormEvent) => {
    ...
    const subtotal = expenseData.items.reduce((sum, item) => sum + item.amount, 0);
    ...
    addExpense(currentGroup.id, {
      ...
      items: expenseData.items.filter(item => item.itemName.trim() && item.amount > 0),
  ...
  const handleUpdateItem = (index: number, field: 'personId' | 'itemName' | 'amount', value: string | number) => {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          {groups.map((group) => (
            <Card
              key={group.id}
              className={selectedGroup === group.id ? 'ring-2 ring-primary' : ''}
            >
              <CardHeader>
              ...
              <CardContent>
                <div className="text-sm text-muted-foreground">
                  {group.people.length} orang • {group.expenses.length} pengeluaran
                </div>
              </CardContent>
            </Card>
          ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <form onSubmit={handleAddExpense} className="space-y-4">
                  ...
                  <div>
                    <Label className="text-sm mb-2 block">Item per orang:</Label>
                    <div className="space-y-2">
                      {expenseData.items.map((item, index) => {
                        const person = currentGroup.people.find(p => p.id === item.personId);
                        return (
                          <div key={index} className="flex gap-2 items-center p-2 border rounded bg-muted/30">
                            <Select
                              value={item.personId}
                              onChange={(e) => handleUpdateItem(index, 'personId', e.target.value)}
                              className="flex-1"
                            >
                            ...
                            <Input
                              placeholder="Nama item (contoh: Ayam + Nasi)"
                              value={item.itemName}
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                if (value === '' || /^\d+$/.test(value)) {
                                  handleUpdateItem(index, 'amount', value === '' ? 0 : Number(value));
                                }
                              }}
                              className="w-32"
                            />
                    ...
                    {expenseData.items.length > 0 && (() => {
                      const subtotal = expenseData.items.reduce((sum, item) => sum + item.amount, 0);
                      const taxAmount = expenseData.includeTax && expenseData.taxPercentage > 0
                      ...
                      return (
                        <div className="mt-2 p-3 bg-primary/10 rounded space-y-1">
                          ...
                          {expenseData.includeTax && expenseData.taxPercentage > 0 && (
                            <div className="flex justify-between text-sm">
                              <span>Pajak ({expenseData.taxPercentage}%):</span>
                              <span className="font-medium text-primary">+ Rp {Math.round(taxAmount).toLocaleString('id-ID')}</span>
                            </div>
                          )}
                          <div className="flex justify-between text-sm font-semibold border-t pt-1 mt-1">
                            <span>Total yang dibayar:</span>
                            <span className="text-primary">Rp {Math.round(total).toLocaleString('id-ID')}</span>
                          </div>
                        </div>
                      );
                    })()}
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        onChange={(e) => setExpenseData({ ...expenseData, includeTax: e.target.checked })}
                    ...
                    {expenseData.includeTax && (
                      <div className="flex gap-2 items-center">
                        <Label className="text-sm whitespace-nowrap">Persentase pajak:</Label>
                        <Input
                          type="text"
                          inputMode="decimal"
                          value={expenseData.taxPercentage === 0 ? '' : (expenseData.taxPercentage || '')}
                          onChange={(e) => {
                            const value = e.target.value;
                  ...
                  <Button 
                    ...
                    disabled={
                      ...
                      !expenseData.paidBy || 
                      expenseData.items.some(item => !item.itemName.trim() || item.amount <= 0) ||
                      (expenseData.includeTax && expenseData.taxPercentage <= 0)
                    }
                  >
            ...
            <Card className="lg:col-span-2">
              <CardHeader>
                ...
                <CardDescription>Pengeluaran yang sudah tersimpan</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {currentGroup.expenses.length > 0 ? (
                    currentGroup.expenses.map((expense) => {
                      const paidBy = currentGroup.people.find(p => p.id === expense.paidBy);
                      const subtotal = expense.items?.reduce((sum, item) => sum + item.amount, 0) || 0;
                      const taxAmount = expense.includeTax && expense.taxPercentage > 0
                        ...
                        <div key={expense.id} className="p-4 border rounded-lg hover:shadow-md transition-shadow">
                          <div className="flex items-start justify-between gap-4">
                              ...
                              <div className="space-y-1 mb-2">
                                {expense.includeTax && expense.taxPercentage > 0 && (
                                  <p className="text-xs text-muted-foreground">
                                    Subtotal: Rp {subtotal.toLocaleString('id-ID')} + Pajak ({expense.taxPercentage}%): Rp {Math.round(taxAmount).toLocaleString('id-ID')}
                                  </p>
                                )}
                                <p className="text-base font-bold text-primary">
                                  Total: Rp {Math.round(expense.totalAmount).toLocaleString('id-ID')}
                                </p>
                              </div>
                              <p className="text-sm text-muted-foreground mb-3">
                                Dibayar oleh: <span className="font-semibold text-foreground">{paidBy?.name}</span>
                              </p>
                              {expense.items && expense.items.length > 0 && (
                                <div className="mt-3 pt-3 border-t space-y-2">
                                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Item per orang:</p>
                                  {expense.items.map((item, idx) => {
                                    ...
                                    return (
                                      <div key={idx} className="text-sm text-foreground pl-3 border-l-2 border-primary/30 py-1">
                                        <span className="font-semibold">{person?.name}:</span> {item.itemName} - 
                                        <span className="font-medium"> Rp {item.amount.toLocaleString('id-ID')}</span>
                                        {expense.includeTax && expense.taxPercentage > 0 && (
                                          <span className="text-muted-foreground">
                                            {' '}(+ pajak: Rp {Math.round(itemTotal - item.amount).toLocaleString('id-ID')}) = 
                                            <span className="font-semibold text-primary"> Rp {Math.round(itemTotal).toLocaleString('id-ID')}</span>
                                          </span>
                                        )}
                                      </div>
                                    );
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => deleteExpense(currentGroup.id, expense.id)}
              ...
              <CardContent>
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="space-y-2">
                      {Object.entries(balances).map(([personId, balance]) => {
                        const person = currentGroup.people.find(p => p.id === personId);
                        const roundedBalance = Math.round(balance * 100) / 100;
                        return (
                          <div key={personId} className="flex items-center justify-between p-3 border rounded bg-card">
                            ...
                            <span className={`font-semibold ${
                              ...
                              {roundedBalance > 0.01 
                                ? `+Rp ${roundedBalance.toLocaleString('id-ID')}` 
                                : roundedBalance < -0.01 
                                ? `-Rp ${Math.abs(roundedBalance).toLocaleString('id-ID')}` 
                                : 'Rp 0'}
                            </span>
                          </div>
                        );
                  ...
                  {settlements.length > 0 && (
                    <div>
                      <Label className="mb-2 block font-semibold">Kesimpulan Penyelesaian:</Label>
                      <div className="space-y-2">
                        {settlements.map((settlement, idx) => {
                          const from = currentGroup.people.find(p => p.id === settlement.from);
                          const to = currentGroup.people.find(p => p.id === settlement.to);
                          return (
                            <div key={idx} className="flex items-center gap-2 p-3 border-2 border-primary/20 rounded bg-primary/5">
                              <span className="font-semibold text-lg">{from?.name}</span>
                  ...
                  {settlements.length === 0 && Object.keys(balances).length > 0 && (
                    <div className="text-center py-4 text-muted-foreground">
                      <p className="text-sm">Semua saldo sudah seimbang (tidak ada hutang)</p>
                    </div>
                  )}
                </div>
              </CardContent>
        ...
        {groups.length === 0 && (
          <Card className="text-center py-12">
            <CardContent>
              ...
              <p className="text-muted-foreground">Belum ada group yang dibuat</p>
            </CardContent>
          </Card>
        )}
      </div>
